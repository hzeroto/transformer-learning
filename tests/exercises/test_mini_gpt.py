...
class TestPostLNBlock(unittest.TestCase):
    def test_model_selects_the_requested_block_style(self):
        pre = make_model(norm_style="pre")
        post = make_model(norm_style="post")
        self.assertTrue(all(type(block) is TransformerBlock for block in pre.blocks))
        self.assertTrue(all(type(block) is PostLNBlock for block in post.blocks))
        self.assertIsInstance(pre.final_norm, LayerNorm)
        self.assertIsNone(post.final_norm)

    def test_post_order_and_gradients_match_independent_composition(self):
        """教师参照只用于测试；不调用待测 PostLNBlock.forward 构造期望值。"""
        from exercises.ex007_multi_head_attention.attention import multi_head_self_attention

        torch.manual_seed(21)
        block = PostLNBlock(4, 2, 8, p=0.0, dtype=DTYPE)
        ...
        # 使用已验收的 Attention，独立展开两个相加后归一化的数学参照。
        a, _ = multi_head_self_attention(
            x, block.Wq, block.Wk, block.Wv, block.Wo, valid, block.num_heads
        )
        u = torch.nn.functional.layer_norm(
            x + a, (4,), block.norm1.gamma, block.norm1.beta, block.norm1.eps
        )
        d = torch.relu(u @ block.ffn.W1 + block.ffn.b1) @ block.ffn.W2 + block.ffn.b2
        expected = torch.nn.functional.layer_norm(
            u + d, (4,), block.norm2.gamma, block.norm2.beta, block.norm2.eps
        )
        actual = block(x, valid)
        torch.testing.assert_close(actual, expected, atol=ATOL, rtol=RTOL)
        self.assertTrue(torch.equal(x, before))
        upstream = torch.randn_like(actual)
        actual_grads = torch.autograd.grad(actual, (x, *params), upstream)
        expected_grads = torch.autograd.grad(expected, (x, *params), upstream)
        for actual_grad, expected_grad in zip(actual_grads, expected_grads):
            torch.testing.assert_close(actual_grad, expected_grad, atol=ATOL, rtol=RTOL)

    def test_post_uses_both_dropout_modules_and_preserves_mode(self):
        block = PostLNBlock(4, 2, 8, p=0.4, dtype=DTYPE)
        x = torch.randn(1, 3, 4, dtype=DTYPE)
        valid = torch.ones(1, 3, dtype=torch.bool)
        seen = []
        handles = [
            drop.register_forward_hook(lambda module, _args, _out: seen.append(module))
            for drop in (block.drop1, block.drop2)
        ]
        try:
            block.train()
            block(x, valid)
            self.assertEqual(seen, [block.drop1, block.drop2])
            self.assertTrue(all(module.training for module in block.modules()))
            block.eval()
            first = block(x, valid)
            rng_before = torch.get_rng_state().clone()
            second = block(x, valid)
            self.assertTrue(torch.equal(first, second))
            self.assertTrue(torch.equal(rng_before, torch.get_rng_state()))
            self.assertTrue(all(not module.training for module in block.modules()))
        finally:
