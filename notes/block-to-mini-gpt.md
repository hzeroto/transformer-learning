...
本页数值来自导出包记录的实验（PyTorch 2.14.0、CPU float64），导出说明称实验直接调用
`exercises/ex008_transformer_block/block.py`。导出包未附完整实验脚本，本文保留原始数值作为
示例记录，不将其视为本地重新复现的结果。教师核对不代表你的实现已通过新的验收。
...
交叉熵为 `3.014379`，而 `ln(20) = 2.995732` 是对 20 个 token 均匀分配概率时的交叉熵。
未训练的模型接近随机，符合预期；这个数字只说明起点正常，不代表任何学习能力。

关键在于**为什么可以并行**：位置 0 的预测不能看见位置 1 的输入，否则就是抄答案。
保证这一点的正是块内的因果 mask。导出包另用长度为 5 的输入做扰动对照，位置从 0 编号：
...
这次实验中位置 0–3 的变化为 0。在关闭 Dropout、参数固定、位置编号与可见范围相同的条件下，
整段前向的各位置结果与分别输入对应前缀的结果在数学上一致；浮点实现按容差比较。
训练时也可以利用因果 mask 并行计算 T 个预测位置；启用 Dropout 后，独立前向的随机标记
可能不同，不要求逐次输出相等。这是训练能高效的原因，也是 `learning-mechanics.next-token` 里
...
与喂 4 个 token 时的位置 2 的 logits ——  在浮点容差内一致
...
- 生成时若一次只送入新 token，位置偏移必须由调用方正确传入，而不是默认从 0 开始。
  同时必须保留历史上下文供 Attention 读取，例如缓存历史 K/V；仅修正位置不能代替历史输入。
  当前未实现缓存，生成时仍应重新输入完整前缀。
...
尺度总体上升，但图中第 8、12 层有局部回落；第 12 层约为第 1 层的 2.85 倍。
...
这里 RMS 约为 1 的结果对应初始 `gamma=1`、`beta=0`，且方差远大于 `eps`；
训练改变这两个参数后，最终输出的 RMS 不保证等于 1。

这解释了一个容易被当成「惯例」的细节：Post-LN 的每个子层输出都过了 LN，
末尾已有这一次；Pre-LN 的块输出没有这一步，因此本课在末尾补上。它是常见结构选择，
不是说省略后模型就无法计算。两种排列的差别不止于「LN 放前面还是后面」，
还包括**堆栈末端的状态**。
...
另一份实现为了省算力，先处理长度为 3 的初始前缀（位置为 0、1、2），之后每次只把
**新追加的那一个 token** 送入模型。假设历史上下文已通过缓存正确保留，因果可见范围也正确，
本题只检查位置编号；缓存实现留待后续学习。这份实现每次仍取 `P[0:1]`，也不会报错。

第二份的位置信息错在哪？当初始前缀之后追加的第 5 个 token 被送入模型时，
它实际应当使用 `P` 的哪一行（从 0 编号，并假设位置表足够长）？
