...
> 本文保留的是进入多头前的复习视角，下面的“尚未完成”是当时的阶段说明，不是当前进度。
> MHA 已完成后，覆盖 Block、Dropout 与训练生命周期的最新串联见
> [参数与数据流：谁在计算，谁在被更新？](parameter-data-flow.md)。当前能力仍以 learning/progress.json 为准。
