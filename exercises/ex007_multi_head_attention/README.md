...
布局阶段的实际实现通过全部 11 项布局测试及当时全仓 125 项回归测试，无跳过。
已验证独立元素映射、连续/转置/步长切片输入、头数校验、输入不被修改和梯度回传。
第一部分验收的是拆头与合头；完整 MHA 的实际验收见下方，不推定学习者独立设计了全部测试。

教师另以同一组元素检查实际存储：拆头后立即合头可以共享原存储；
先生成连续的 `(B,H,T,Dh)` 输入再合头，该样例中的 `reshape` 会复制，元素仍然正确。
这只是教师诊断，不作为学习者已独立解释复制条件的证据；该能力留待后续多头集成中验证。

第二部分的 multi_head_attention 与 multi_head_self_attention 也已由学习者完成。
本次直接复跑仓库中的真实实现，15 项 MHA 综合测试和全仓 140 项回归全部通过、无跳过，
不是教师临时参照的结果。两个接口的前向、输入/参数梯度、非连续输入、
不同 Tq/Tk、每头 sqrt(Dh)、mask 广播与未来/PAD/前缀可见性均已验收；
教师本轮未修改学习者答案。
implementation.pytorch-mha 已有掌握记录；本次补充完整回归证据，不重复上报新的掌握。
完整语言模型、GPU 性能及尚无独立解释证据的概念边界不由测试通过自动推定。

多头原理收尾测验也已通过，attention.multi-head 已上报掌握：学习者已解释共享权重不能
一般地代替不同头的读取分布，设计输入宽度与投影总宽度不同的合法配置，并在题面明确
对象和完整 shape 后正确构造跨头选择特征的 Wo。下一步进入 Transformer Block，
不重做已通过的多头实现；布局复制条件和尺度统计观察仍由各自节点单独记录。
...
第二部分已将完整多头计算、mask 的 head 轴、输出投影 Wo 合成一份实现并通过验收。
...
最初的空框架会出现两个明确的未实现失败，三个行为测试类暂时跳过；当前实现已无失败或跳过。
...
## 第二部分：完整多头 Attention（实现已验收）

先看[完整数据流与 Wo 讲义](../../notes/multi-head-attention.md)。
本部分的两个接口均位于 attention.py，已完成：
...
组合实现 review 已通过。后续进入 Transformer Block，并在整合中继续核验概念边界，
不重复已通过代码，也不为每个计算步骤各交一次作业。
